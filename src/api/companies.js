...
export default {
    async getCompanies() {
        const url = "companies?sort=-categories&expand=requests,contacts.emails,contacts.phones,contacts.contactComments,broker,companyGroup,consultant,consultant.userProfile,productRanges,categories,files";
        let data = false;
        await axios
            .get(url)
            .then((Response) => {
    ...
    async getCompany(id) {
        const url = "companies/" + id + "?expand=contacts.emails,contacts.phones,contacts.websites,contacts.contactComments,broker,companyGroup,consultant,consultant.userProfile,categories,productRanges,files,dealsRequestEmpty.consultant.userProfile,dealsRequestEmpty.offer.generalOffersMix,dealsRequestEmpty.competitor";
        let data = false;
        await axios
            .get(url)
            .then((Response) => {
    ...
    async searchCompanies(query) {
        ...
        let url = "companies?" + query + "&expand=requests.timelines.timelineSteps,contacts.emails,contacts.phones,contacts.contactComments,broker,companyGroup,consultant,consultant.userProfile,productRanges,categories,files,mainContact.phones,mainContact.emails";
        let data = false;
        await axios
            .get(url)
            .then((Response) => {
