...
export default {
    ...
    async find(params) {
        const response = await axios.get(URL, { params });
        return responseToData(response)[0] ?? null;
    },
    async getByChatMemberId(chatMemberId) {
        const response = await axios.get(`${URL}/chat-members/${chatMemberId}`);
        return responseToData(response);
    }
};
