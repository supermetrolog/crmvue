...
const emit = defineEmits<{
    ...
    updated: [optionId: number, option: AttributeOption];
    delete: [option: AttributeOption];
    close: [];
}>();

const props = defineProps<{ formData: AttributeOption | null }>();

const { form, isEditMode } = useFormData(
    reactive<Omit<CreateAttributeOptionDto, 'attribute_id'> & { id: number }>({
        id: dayjs().unix(),
        label: '',
        value: '',
        sort_order: 10
