        ...
        <Tabs :options="{ useUrlFragment: false }">
            <Tab name="Характеристики">
                <div class="ObjectHoldingsTabs-content">
                    <PropertyList>
                        <PropertyListItem v-for="(prop, idx) in holdingsProperties[0]" :key="prop.name + idx"
                            :name="prop.name" :value="prop.value" :unit="prop.unitType" />
                    </PropertyList>
                    <PropertyList>
                        <PropertyListItem v-for="(prop, idx) in holdingsProperties[1]" :key="prop.name + idx"
                            :name="prop.name" :value="prop.value" :unit="prop.unitType" />
                    </PropertyList>
                    <PropertyList>
                        <PropertyListItem v-for="(prop, idx) in holdingsProperties[2]" :key="prop.name + idx"
                            :name="prop.name" :value="prop.value" :unit="prop.unitType" />
                    </PropertyList>
                    <PropertyList>
                        <PropertyListItem v-for="(prop, idx) in holdingsProperties[3]" :key="prop.name + idx"
                            :name="prop.name" :value="prop.value" :unit="prop.unitType" />
                    </PropertyList>
                </div>
            </Tab>
