<template>
    <div class="d-flex gap-1 align-items-start">
        <div class="messenger-quiz-form-request-picker-element flex-grow-1">
            ...
            <div v-if="request.action && request.description">
                <p class="text-grey">Комментарий:</p>
                <p class="fs-2">{{ request.description }}</p>
            </div>
        </div>
        <div class="d-flex gap-1">
            <Chip :class="answerClass">{{ answerText }}</Chip>
            <Chip v-if="request.action" class="dashboard-bg-light">{{ actionText }}</Chip>
        </div>
    </div>
</template>
<script setup>
...
const actionText = computed(() => {
    if (isNotNullish(props.request.action)) return actonOptions[props.request.action];
    return null;
});
</script>
