    ...
    defineMarker() {
      ...
      if (this.balloonTemplate) {
        balloonContentLayout =
          this.$parent.$options.static.myMap.templateLayoutFactory.createClass(
      ...
      if (
        this.icon &&
        ["default#image", "default#imageWithContent"].includes(this.icon.layout)
      ) {
      ...
      const mapMarker = utils.createMarker(
        obj,
        this.useObjectManager,
        this.$emit
      );
      if (!this.useObjectManager) {
        const events = this.markerEvents.length
          ? this.markerEvents
          : defaultMarkerEvents;
