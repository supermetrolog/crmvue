            ...
            <div class="col-12">
                <DashboardCard>
                        ...
                        >
                            <template #extra>
                                ...
                                >
                                    ...
                                    <span class="ml-1">Обновить тэги</span>
                                </DashboardChip>
                            </template>
                        </SettingsFormHeader>
                    </template>
                    <div class="d-flex flex-wrap gap-2 mb-2">
                        ...
                        <template v-else>
                            <Spinner v-if="tagsIsLoading" class="small mx-auto" />
                            <EmptyLabel v-else>Список тэгов пуст..</EmptyLabel>
                        </template>
                    </div>
                </DashboardCard>
            </div>
...
import FormModalTaskTag from '@/components/Forms/FormModalTaskTag.vue';
...
const fetchTags = async () => {
    ...
    const response = await api.taskTag.all();
    if (response) tags.value = response;
...
const deleteTag = async tag => {
    ...
    const deleted = await api.taskTag.delete(tag.id);
    if (deleted) {
